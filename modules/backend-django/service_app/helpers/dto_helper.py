...
def order_to_dto(order) -> OrderDTO:
    return OrderDTO(
        ...
        user=order.user if order.user is not None else None,
        ...
        incidents=order.incidents if order.incidents is not None else None,
        numberOfBags=order.number_of_bags if order.number_of_bags is not None else None,
...
# Process to DICT
def dispatch_order_to_dict(order_dict) -> dict:
    dispatch_data = [
        "id",
        "code",
        "origin",
    ...
    return {k: v for k, v in order_dict.items() if k in dispatch_data}


def preparation_order_to_dict(order_dict) -> dict:
    preparation_data = [
        "id",
        "code",
        "origin",
    ...
    return {k: v for k, v in order_dict.items() if k in preparation_data}
